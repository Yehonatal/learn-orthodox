...
const TRANSLATIONS = {
  en: {
    pageTitle: "Orthodox 101",
    subtitle: "Basic Teachings & Pillars of Faith",
    searchPlaceholder: "Search teachings...",
    loadingLessons: "Loading teachings...",
    selectPrompt: "Select a chapter to begin your journey of faith.",
    aiCompanion: "AI Theological Companion",
    aiDescription: "Ask questions regarding this chapter's theology, history, or biblical references.",
    ...
    suggestedQuestions: [
      "What is the biblical basis for this chapter?",
      "How is this teaching reflected in the Liturgy?",
      "Can you summarize this in simpler terms?",
      "What did the early Church Fathers write about this?"
    ]
  },
  am: {
    pageTitle: "መሠረተ እምነት",
    subtitle: "የኦርቶዶክስ ተዋሕዶ መሠረታዊ ትምህርቶች",
    searchPlaceholder: "ትምህርቶችን እዚህ ይፈልጉ...",
    loadingLessons: "ትምህርቶች በመጫን ላይ ናቸው...",
    selectPrompt: "ለመማር መጀመር ከጎን ካሉት ምዕራፎች አንዱን ይምረጡ።",
    aiCompanion: "የነገረ መለኮት ረዳት (AI)",
    aiDescription: "ስለዚህ ምዕራፍ ሥነ-መለኮት፣ ታሪክ ወይም መጽሐፍ ቅዱሳዊ ጥቅሶች ይጠይቁ።",
    chatPlaceholder: "ስለዚህ ትምህርት ጥያቄዎን እዚህ ይጻፉ...",
    ...
    askCompanionBtn: "የነገረ መለኮት ረዳት ጠይቅ",
    suggestedQuestions: [
      "የዚህ ምዕራፍ መጽሐፍ ቅዱሳዊ መሠረት ምንድን ነው?",
      "ይህ ትምህርት በሥርዓተ ቅዳሴ ላይ እንዴት ይገለጻል?",
      "ይህን ትምህርት በቀላል ቃላት ማጠቃለል ትችላለህ?",
      "የጥንት አባቶች (ሊቃውንት) ስለዚህ ጉዳይ ምን ብለዋል?"
    ]
  }
};

export default function Orthodox101Page() {
  const [lang, setLang] = useState<'en' | 'am'>('en');
  const t = TRANSLATIONS[lang];

  const [lessons, setLessons] = useState<OrthodoxLesson[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedLesson, setSelectedLesson] = useState<OrthodoxLesson | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      const data = await getOrthodoxLessons();
      setLessons(data);
      if (data.length > 0) {
  ...
  }, [chatHistory, selectedLesson, showAIChat]);
  ...
  const currentMessages = useMemo(() => {
    ...
    return chatHistory[selectedLesson.id] || [];
  }, [chatHistory, selectedLesson]);

  // Send message to AI
  const handleSendMessage = async (textToSend?: string) => {
    const text = (textToSend || inputText).trim();
    if (!text || !selectedLesson || sendingChat) return;
    ...
    const updatedHistory = [...(chatHistory[lessonId] || []), newUserMessage];
    setChatHistory(prev => ({
      ...prev,
    ...
    try {
      ...
      const res = await fetch('/api/lessons/chat', {
        ...
        body: JSON.stringify({
          ...
          lessonText: selectedLesson.description.slice(0, 4000), // send snippet context
      ...
      setChatHistory(prev => ({
        ...prev,
        [lessonId]: [...updatedHistory, assistantMessage]
      }));
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-bg-parchment text-text-ink flex flex-col justify-between selection:bg-accent-gold/20 relative overflow-hidden font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="text-center flex-grow flex flex-col justify-center items-center">
            <span className="block font-serif font-extrabold text-[11px] sm:text-xs text-text-ink tracking-wider uppercase">
              {lang === "am" ? "መሠረተ እምነት ትምህርቶች" : "Orthodox 101 lessons"}
            </span>
            <span className="block text-[8px] text-accent-crimson tracking-widest uppercase font-bold mt-0.5">
              {lang === "am" ? "መሠረታዊ የሃይማኖት ትምህርቶች" : "Basic Christian Teachings"}
            </span>
          </div>
      ...
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 flex-grow relative z-10 flex flex-col">
        {loading ? (
          <div className="flex-grow flex flex-col items-center justify-center py-24">
            ...
            <p className="text-stone-500 font-serif text-sm italic">{t.loadingLessons}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch flex-grow">
            ...
            <aside className="lg:col-span-4 flex flex-col space-y-4">
              ...
              <div className="border border-accent-gold/25 bg-bg-alabaster/40 flex-grow max-h-[70vh] lg:max-h-[64vh] overflow-y-auto rounded-none soft-shadow relative">
                ...
                <div className="divide-y divide-accent-gold/15">
                  {filteredLessons.map((lesson, idx) => {
                    const active = selectedLesson?.id === lesson.id;
                    const geezNum = GEEZ_NUMERALS[idx] || `${idx + 1}`;
                    
                    return (
                      <button
                        key={lesson.id}
                        onClick={() => setSelectedLesson(lesson)}
                        className={`w-full text-left p-3.5 transition-all duration-200 cursor-pointer flex items-start gap-3 rounded-none ${
                          active 
                            ? 'bg-bg-alabaster border-l-4 border-accent-gold' 
                            : 'hover:bg-bg-alabaster/60'
                        }`}
                      >
                        <span className="font-serif text-xs font-bold text-accent-crimson bg-bg-parchment border border-accent-gold/20 px-2 py-0.5 rounded-none soft-shadow min-w-[24px] text-center">
                          {geezNum}
                        </span>
                        <div className="flex-grow min-w-0">
                          <h4 className={`text-[12px] font-serif font-extrabold leading-tight text-text-ink ${active ? 'text-accent-indigo font-black' : ''}`}>
                            {lesson.title}
                          </h4>
                          <p className="text-[10px] text-stone-550 truncate mt-1">
                            {lesson.description.slice(0, 60)}...
                          </p>
                        </div>
                        <ChevronRight className={`h-4.5 w-4.5 text-accent-gold/40 self-center ${active ? 'text-accent-gold translate-x-0.5' : ''}`} />
                      </button>
                    );
                  })}
            ...
            {/* Column 2: Lesson Reading Scroll Parchment (Span 8) */}
            <section className="lg:col-span-8 flex flex-col">
              {selectedLesson ? (
                <div className="bg-bg-parchment border border-accent-gold/25 p-6 md:p-8 flex flex-col flex-grow relative rounded-none soft-shadow overflow-hidden min-h-[50vh] lg:max-h-[72vh]">
                  ...
                  <div className="border-b border-accent-gold/20 pb-4 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 relative">
                    <div>
                      <span className="ui-label text-[9px] font-bold text-accent-crimson tracking-widest block mb-1">
                        {lang === 'am' ? 'የጥናት ምዕራፍ' : 'LECTURE CHAPTER'}
                      </span>
                  ...
                  <div className="flex-grow overflow-y-auto pr-2 relative z-10 font-serif text-xs md:text-sm leading-relaxed text-text-ink whitespace-pre-line space-y-4 max-h-[55vh]">
      ...
      {/* Slide-out AI Theological Chat Companion (Drawer UI) */}
              ...
              <div className="flex-grow overflow-y-auto space-y-4 mb-4 pr-1 max-h-[70vh]">
                ...
                <div className="flex items-start gap-2.5 max-w-[85%]">
                  ...
                  <div className="bg-bg-parchment border border-accent-gold/15 p-3 text-xs leading-relaxed text-text-ink font-serif rounded-none soft-shadow">
                    {lang === 'am' 
                      ? `እንኳን ደህና መጡ! ስለ "${selectedLesson.title}" ምን ማወቅ ይፈልጋሉ? ከዚህ በታች ጥያቄዎን መጠየቅ ይችላሉ።`
                      : `Welcome! What would you like to learn about "${selectedLesson.title}"? Feel free to ask any theological or historical questions.`
                    }
                  </div>
                </div>
